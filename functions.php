<?php
/**
 * Rakesh_WP functions and definitions
 *
 ...
 */
function rakesh_wp_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on Rakesh_WP, use a find and replace
		* to change 'rakesh_wp' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'rakesh_wp', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'rakesh_wp' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'rakesh_wp_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'rakesh_wp_setup' );
 ...
 */
function rakesh_wp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'rakesh_wp_content_width', 640 );
}
add_action( 'after_setup_theme', 'rakesh_wp_content_width', 0 );
 ...
 */
function rakesh_wp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'rakesh_wp' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'rakesh_wp' ),
...
add_action( 'widgets_init', 'rakesh_wp_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function rakesh_wp_scripts() {
	wp_enqueue_style( 'rakesh_wp-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'rakesh_wp-style', 'rtl', 'replace' );

	wp_enqueue_script( 'rakesh_wp-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'rakesh_wp_scripts' );
...
function rakesh_assets() {

    // CSS
    wp_enqueue_style('bootstrap', get_template_directory_uri() . '/assets/css/bootstrap.min.css');
    wp_enqueue_style('animate', get_template_directory_uri() . '/assets/lib/animate/animate.min.css');
    wp_enqueue_style('owl', get_template_directory_uri() . '/assets/lib/owlcarousel/assets/owl.carousel.min.css');
...
// Enqueue Bootstrap 5 JavaScript bundle (includes Popper.js)
    ...
    wp_enqueue_script(
        ...
        array('jquery','wow','owl-carousel','isotope','counterup','easing'),
        null,
        true
    );
...
add_action('wp_enqueue_scripts', 'rakesh_assets');
